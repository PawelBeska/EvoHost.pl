...
<script>
    $(document).ready(function(){
        function load_unseen_notification()
        {
            ...
            $.ajax({
...

                {
                    ...
                    $('.show-notification.user-notification').append(' <li>\n' +
                        '                        <div class="media">\n' +
                    ...
                    if (data === undefined || data.length == 0) {
                        ...
                        $('.ti-bell').html('<span class="badge count">'+data.length+'</span>');
                    }
                }
            });
        }
        load_unseen_notification();
        $('.show-notification.user-notification').on('click', '.notification_remove', function (e) {
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            ...
            $.ajax({
                ...
                success:function(data)
                {
                    if(data>0)
                    {
                        $('.ti-bell').html('<span class="badge count">'+data+'</span>');
                    }else{
                        $('.ti-bell').html('');
                    }
                    load_unseen_notification();
        ...
        $(document).on('click', 'li.header-notification', function(){
